...
class RecentsContentResolver(
...
) : BaseContentResolver<RecentRecord>(filter, contentResolver) {
    ...
    override val selection: String
        get() {
            ...
            currentFilter?.let { selection.addString("(${CallLog.Calls.CACHED_NAME} LIKE '%$it%' OR ${CallLog.Calls.NUMBER} LIKE '%$it%')") }
            return selection.build()
        }


    @SuppressLint("Range")
    override fun convertCursorToItem(cursor: Cursor): RecentRecord {
        return RecentRecord(
